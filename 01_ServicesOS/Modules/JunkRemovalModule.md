# ServicesOS Junk Removal Module

## Purpose

The Junk Removal Module is a future ServicesOS vertical module.

It should reuse the ServicesOS Core Platform and only add the industry-specific workflows, fields, checklists, pricing logic, training, and dashboard widgets needed for junk removal businesses.
...
```javascript
export const serviceTypes = ['small_load', 'half_truck', 'full_truck', 'appliance_removal', 'furniture_removal', 'construction_debris', 'yard_waste'];
```
...
```javascript
const junkRemovalJobDetails = {
  tenantId: "tenant_abc",
  jobId: "job_001",
  moduleId: "junk-removal",

  truckLoadEstimate: "half",
  heavyItems: true,
  itemPhotos: ["photo_001", "photo_002"],
  crewSize: 2,

  dumpFeeEstimate: 45,
  donationPossible: true,
  recyclingRequired: false,
...
```text
Customer uploads item photos → load size estimated → crew size assigned → pickup scheduled → employee confirms items → optional extra items added → dump/donation tracked → payment collected.
```
