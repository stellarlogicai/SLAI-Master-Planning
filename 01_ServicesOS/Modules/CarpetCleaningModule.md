# ServicesOS Carpet Cleaning Module

## Purpose

The Carpet Cleaning Module is a future ServicesOS vertical module.

It should reuse the ServicesOS Core Platform and only add the industry-specific workflows, fields, checklists, pricing logic, training, and dashboard widgets needed for carpet cleaning businesses.
...
```javascript
export const serviceTypes = ['room_cleaning', 'stain_treatment', 'pet_odor_treatment', 'stairs', 'upholstery', 'rug_cleaning', 'deep_extraction'];
```
...
```javascript
const carpetCleaningJobDetails = {
  tenantId: "tenant_abc",
  jobId: "job_001",
  moduleId: "carpet-cleaning",
  ...
  equipmentRequired: ["extractor", "hose", "fan"],
  dryTimeHours: 6,
...
```text
Customer selects rooms → stains/pets noted → estimate generated → technician assigned → equipment checklist completed → before photos → cleaning completed → after photos → dry time instructions sent → payment collected.
```
